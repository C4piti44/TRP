...
class SwerveSubsystem(Subsystem):
    def __init__(self) -> None:
        ...
        self.odometer = SwerveDrive4PoseEstimator(Constants.Swerve.oldSwerveKinematics,
                    Rotation2d.fromDegrees(self.getHeading()),
                    (self.frontLeft.get_position(), self.frontRight.get_position(),
                    self.backRight.get_position(), self.backLeft.get_position()),
                    Pose2d())
    ...
    def periodic(self) -> None:
        rotation:float = self.getHeading()
        self.module_positions = (self.frontLeft.get_position(),
                                 self.frontRight.get_position(),
                                 self.backRight.get_position(),
                                 self.backLeft.get_position())

        self.odometer.update(Rotation2d.fromDegrees(rotation), self.module_positions)
    ...
    def drive(self, xSpeed: float, ySpeed: float, tSpeed: float, fieldOriented: bool = True) -> None:
        ...
        moduleState = Constants.Swerve.oldSwerveKinematics.toSwerveModuleStates(cSpeed, Translation2d())
